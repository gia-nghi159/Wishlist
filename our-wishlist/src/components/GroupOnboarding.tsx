...
export default function GroupOnboarding({ onGroupLinked }: OnboardingProps) {
  ...
  return (
    <div className="max-w-md mx-auto my-16 p-8 bg-white border border-gray-200 rounded-2xl shadow-xl">
      <h2 className="text-2xl font-bold text-[#618264] mb-2">Welcome to Our Wishlist! 🎁</h2>
      ...
      <form onSubmit={handleCreate} className="mb-8 pb-8 border-b border-gray-100">
        <label className="block text-xs font-bold text-[#618264] uppercase tracking-wider mb-2">Option 1: Start a New List</label>
        <div className="flex gap-2">
          <input 
            ...
            className="flex-1 px-4 py-2 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-700"
            disabled={isJoining}
          />
          <button 
            type="submit" 
            className="bg-[#618264] text-white px-4 py-2 rounded-xl text-sm font-semibold hover:bg-[#4A6A4C] disabled:opacity-50"
      ...
      <form onSubmit={handleJoin} className="mb-6">
        <label className="block text-xs font-bold text-[#618264] uppercase tracking-wider mb-2">Option 2: Join Your Partner's List</label>
        <div className="flex gap-2">
          <input 
            type="text" 
      ...
      <button 
        onClick={() => signOut()} 
        className="w-full text-center text-md text-gray-400 hover:text-[#618264] mt-4 underline"
      >
        Log out
      </button>
    </div>
